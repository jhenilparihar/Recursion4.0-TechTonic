    ...
    return (
        <div className="w-[500px] h-auto p-[20px] bg-[#12131A] text-white flex flex-col gap-y-[20px] rounded-lg m-auto mt-20">
            ...
            <div className="">
                ...
                {showCreatePass && <div className="flex flex-row flex-wrap w-[80%] justify-between gap-y-[5px] mt-[10px]">
                    <SelectImages images={IMAGES} setPass={setPass} setPassStatus={setPassStatus} setShowCreatePass={setShowCreatePass} setShowConfirmPass={setShowConfirmPass}/>
                </div>}
            </div>

            {pass.length > 0 && <div className="">
                <div className="flex flex-row">
                    <button className="min-w-[200px] bg-primaryBlue py-[8px] px-[24px] rounded-lg text-white font-inter font-medium self-start border-[1px]"
                        onClick={() => {setShowConfirmPass(true)}}
                    >Confirm Password</button>

                    {confirmPassStatus && <div className="ml-auto">
                        <img src="greentick.png" className="w-[20px] h-[20px]"/>
                    </div>}
                </div>
                {showConfirmPass && <div className="flex flex-row flex-wrap w-[80%] justify-between gap-y-[5px] mt-[10px]">
                    <ConfirmSelectedImages images={IMAGES} pass={pass} setShowConfirmPass={setShowConfirmPass} setPassStatus={setPassStatus} setConfirmPassStaus={setConfirmPassStatus}/>
                </div>}
            </div>}

            {confirmPassStatus && <button className="m-auto min-w-[200px] bg-primaryBlue py-[8px] px-[24px] rounded-lg text-white font-inter font-medium self-start border-[1px]"
